...
public class FruitClick : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Make sure the fruit has a 2D collider so OnMouseDown can detect clicks.
    ...
    private void Update()
    {
        // Try again in case the GameManager was added later.
        if (gameManager == null)
        {
            EnsureGameManagerExists();
        }
    ...
    {
        // Reuse the GameManager if the scene already has one.
        gameManager = FindObjectOfType<GameManager>();

        // If the scene has no GameManager yet, create one automatically.
        if (gameManager == null)
        {
    ...
    {
        // Add a simple BoxCollider2D automatically if the object has no collider yet.
